(function () {
  window.emmet = function (selector, parent) {
    if (typeof selector == "string") {
      ...
      var Element = function (
        ...
        this.check = {
          get tagName() {
            return typeof that.tagName == "string"
              ? that.tagName.match(/^[a-zA-Z0-9\-_]+$/)
                ? true
                : false
              : false;
          },
          get id() {
            return typeof that.id == "string"
              ? that.id.match(/^[a-zA-Z0-9\-_]+$/)
                ? true
                : false
              : false;
          },
          get className() {
            return typeof that.className == "string"
              ? that.className.match(/^[a-zA-Z0-9\-_\s]+$/)
                ? true
                : false
              : false;
          },
            ...
            if (Array.isArray(that.attributes)) {
              for (var i = 0; i < that.attributes.length; i++) {
                if (typeof that.attributes[i] == "object") {
                  if (
                    typeof that.attributes[i].name != "string" ||
                    typeof that.attributes[i].value != "string"
                  )
                    return false;
                } else return false;
              }
          ...
          get innerHTML() {
            return typeof that.innerHTML == "string" ? that.innerHTML : false;
          },
          get content() {
            var content = nestFix(that.content);
            if (Array.isArray(content)) {
              for (var i = 0; i < content.length; i++)
                if (!(content[i] instanceof Element)) return false;
            } else return false;
            return true;
          },
        };
      ...
      var escape = function (selector) {
        ...
        for (var i = 0; i < d.length; i++) {
          var count = {
            s: (eSelector.match(new RegExp(regexEscape(d[i].s.c), "g")) || [])
              .length,
            e: (eSelector.match(new RegExp(regexEscape(d[i].e.c), "g")) || [])
              .length,
          };
            ...
            attributes: function (selector) {
              ...
              if (attributes != null) {
                ...
                for (var i = 0; i < attributes.length; i++) {
                  ...
                  attr +=
                    string && attributes[i] == " " ? "&nbsp;" : attributes[i];
                }
                attributes = attr.split(" ");
                for (var i = 0; i < attributes.length; i++) {
                  var match = attributes[i].match(
                    /^([a-zA-Z0-9\-_]+)="([^"]*)"$/
                  );
                  if (match == null)
                    match = attributes[i].match(/^([a-zA-Z0-9\-_]+)=([^"]*)$/);
                  if (match == null)
                    match = attributes[i].match(/^([a-zA-Z0-9\-_]+)$/);
                  if (match != null)
                    eAttributes.push({
                      name: match[1],
                      value: match.length > 2 ? match[2] : "",
                    });
                }
              }
            ...
            className: function (selector) {
              ...
              if (className !== null)
                for (var i = 0; i < className.length; i++)
                  eClassName += className[i].substr(1) + " ";
        ...
        if (multiplicator != null) {
          ...
          for (var i = 0; i < multiplicator; i++) {
            ...
            for (var j = 0; j < numbering.length; j++) {
              var num = numbering[j];
              var number = (
                (num.direction == 1 ? i : multiplicator - i - 1) + num.from
              ).toString();
              for (var k = number.length; k < num.length; k++)
                number = "0" + number;
      ...
      var treeNav = function (selector) {
        var n = (
          selector[0] == "^" ? [] : [selector.match(/^([^\^]+)\^?/)[1]]
        ).concat(selector.match(/\^+[^\^]+/g) || []);
        var nav = [];
        for (var i = 0; i < n.length; i++) {
          var t = n[i].split("^");
          nav.push({
            direction: t.length == 1 ? 0 : t.length,
            selector: t[t.length - 1],
          });
        }
        return nav;
      };
          ...
          for (var i = 0; i < selector.length; i++) {
            if (i == 0) tree.push(elementParse(selector[i]));
              ...
              for (var j = 0; j < nav.length; j++) {
                var p =
                  nav[j].direction > position.length - 1
                    ? position.length - 1
                    : nav[j].direction;
                position = position.splice(0, position.length - p);
                var currentElement = tree[position[0]];
                for (var k = 1; k < position.length; k++)
                  currentElement = currentElement.content[position[k]];
                if (Array.isArray(currentElement)) {
                  for (var k = 0; k < currentElement.length; k++)
                    currentElement[k].content.push(
                      elementParse(nav[j].selector)
                    );
                } else
                  currentElement.content.push(elementParse(nav[j].selector));
                position.push(0);
              }
            }
          }
  ...
  window.emmets = (arrayEmmet, parent) => {
    arrayEmmet = arrayEmmet.map((e) => emmet(e));
    if (parent) {
      if (typeof parent == "string") {
        parent = document.querySelector(parent);
      }
      arrayEmmet.forEach((e) => {
  ...
  window.Node.prototype.emmet = function (selector) {
    return window.emmet(selector, this);
  };
  window.NodeList.prototype.emmet = function (selector) {
    var result = true;
    for (var i = 0; i < this.length; i++)
      result = result && this[i].emmet(selector, this[i]);
    return result;
  };
  return emmet;
})();
